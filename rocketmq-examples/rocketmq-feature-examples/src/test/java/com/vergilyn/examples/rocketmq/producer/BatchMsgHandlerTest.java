...
public class BatchMsgHandlerTest {
	...
	private RocketDefinedGenerator definedGenerator = new RocketDefinedGenerator("batch_msg_handler");
	...
	public void producer() {
		...
		DefaultMQProducer producer = new DefaultMQProducer(definedGenerator.producerGroup());
	...
	public void pushConsumer() {
		DefaultMQPushConsumer consumer = new DefaultMQPushConsumer(definedGenerator.consumerGroup());
		...
		try {
			 ...
			 */
			consumer.subscribe(definedGenerator.topic(), definedGenerator.tag());
	...
	public void pullConsumer() {
		DefaultMQPullConsumer consumer = new DefaultMQPullConsumer(definedGenerator.consumerGroup());
		consumer.setNamesrvAddr(RocketConstants.NAMESRV_ADDR);
		try {
			consumer.start();

			Set<MessageQueue> mqs = consumer.fetchSubscribeMessageQueues(definedGenerator.topic());
			for (MessageQueue mq : mqs) {
				System.out.printf("Consume from the queue: %s%n", mq);
				SINGLE_MQ:
				while (true) {
					try {
						PullResult pullResult = consumer.pullBlockIfNotFound(mq, definedGenerator.tag(),
	...
	private List<Message> createMsg(int size) {
		...
		for (int i = 0; i < size; i++) {
			MessageDto messageDto = MessageDto.builder().id((long) i).str("user-" + i).build();

			Message message = new Message(definedGenerator.topic(),
			                              definedGenerator.tag(),
			                              "key-" + i,
